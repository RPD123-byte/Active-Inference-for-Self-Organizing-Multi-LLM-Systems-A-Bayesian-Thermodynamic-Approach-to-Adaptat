...
def create_graph(server=None, model=None, stop=None, model_endpoint=None, temperature=0, prompt_templates=None):
    graph = StateGraph(AgentGraphState)

    if prompt_templates is None:
        prompt_templates = {
            'planner': planner_prompt_template,
            'selector': selector_prompt_template,
            'reporter': reporter_prompt_template,
            'reviewer': reviewer_prompt_template,
            'router': router_prompt_template
        }
    ...
    graph.add_node(
        ...
        lambda state: PlannerAgent(
            ...
            prompt=prompt_templates['planner']
        )
    )
    ...
    graph.add_node(
        ...
        lambda state: SelectorAgent(
            ...
            prompt=prompt_templates['selector']
        )
    )
    ...
    graph.add_node(
        ...
        lambda state: ReporterAgent(
            ...
            prompt=prompt_templates['reporter']
        )
    )
    ...
    graph.add_node(
        ...
        lambda state: ReviewerAgent(
            ...
            # serp=lambda: get_agent_graph_state(state=state, state_key="serper_latest"),
            prompt=prompt_templates['reviewer']
        )
    )
    ...
    graph.add_node(
        ...
        lambda state: RouterAgent(
            ...
            # reporter_agent=reporter_prompt_template,
            # serp=lambda: get_agent_graph_state(state=state, state_key="serper_latest"),
            prompt=prompt_templates['router']
        )
    )
