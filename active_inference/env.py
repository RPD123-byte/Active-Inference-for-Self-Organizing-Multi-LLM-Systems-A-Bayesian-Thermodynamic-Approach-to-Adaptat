...
class ResearchAgentEnv(Env):
    ...
    def step_test(self, actions):
        ...
        based on the state and actions, with a special high-performing prompt case.
        """
        print(f"\n{'='*50}")
        ...
        # Check for special case [0 30 0]
        state_indices = [np.argmax(s) for s in self._state]
        if state_indices == [0, 30, 0]:
            print("\nSpecial case detected: optimal prompt state")
            observation = [11, 11, 11, None, None, None, 2]
            return observation
        ...
        if actions[0] > 0:  # Prompt action
            ...
            avg_quality = np.mean([observation[i] for i in range(3)]) / 10.0
            if avg_quality < 0.3:
                observation[INFO_STATE_MODALITY_ID] = 0
            ...
            if search_idx < 3:
                info_state_probs = [0.6, 0.3, 0.1]
            elif search_idx < 7:
                info_state_probs = [0.2, 0.5, 0.3]
            else:
                info_state_probs = [0.1, 0.3, 0.6]
                
            observation[INFO_STATE_MODALITY_ID] = rng.choice(3, p=info_state_probs)
