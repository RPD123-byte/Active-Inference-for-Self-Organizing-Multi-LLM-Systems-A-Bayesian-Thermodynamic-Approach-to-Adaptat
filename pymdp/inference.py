...
def update_posterior_states_factorized(A, obs, num_obs, num_states, mb_dict, prior=None, **kwargs):
    ...
    **kwargs: keyword arguments 
    ...
    # Only process non-None observations
    valid_obs = []
    valid_obs_indices = []
    valid_num_obs = []
    valid_A = []
    old_to_new_idx = {}  # Track mapping from old to new indices

    for m in range(num_modalities):
        if obs[m] is not None:
            valid_obs.append(obs[m])
            valid_obs_indices.append(m)
            valid_num_obs.append(num_obs[m])
            valid_A.append(A[m])
            old_to_new_idx[m] = len(valid_obs) - 1

    
    # Create modified mb_dict for valid observations only
    valid_mb_dict = {
        'A_factor_list': [mb_dict['A_factor_list'][m] for m in range(num_modalities) if obs[m] is not None],
        'A_modality_list': [[old_to_new_idx[m] for m in modlist if obs[m] is not None] for modlist in mb_dict['A_modality_list']]
    }
    
    # Process valid observations
    processed_obs = utils.process_observation(valid_obs, len(valid_obs), valid_num_obs)

    if prior is not None:
        prior = utils.to_obj_array(prior)

    return run_vanilla_fpi_factorized(valid_A, processed_obs, valid_num_obs, num_states, valid_mb_dict, prior, **kwargs)
