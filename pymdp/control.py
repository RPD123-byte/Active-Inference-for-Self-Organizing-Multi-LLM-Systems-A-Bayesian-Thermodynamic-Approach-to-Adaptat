...
def update_posterior_policies_factorized(
    ...
    for idx, policy in enumerate(policies):
        ...
        if use_utility:
            G[idx] += calc_expected_utility(qo_pi, C)
            # print(f"Expected utility: {G[idx]}")

        if use_states_info_gain:
            G[idx] += calc_states_info_gain_factorized(A, qs_pi, A_factor_list)
            # print(f"Expected info gain: {G[idx]}")
...
def get_expected_states_interactions(qs, B, B_factor_list, policy):
    ...
    # Predict states for each timestep
    for t in range(n_steps):
        # Initialize next timestep with full copies of current state beliefs
        qs_pi[t+1] = utils.obj_array_from_list([q.copy() for q in qs_pi[t]])
        
        # Update only the factors that are affected by the policy
        for control_factor, action in enumerate(policy[t,:]):
            factor_idx = B_factor_list[control_factor]
...
def get_expected_obs_factorized(qs_pi, A, A_factor_list):
    ...
    # print("\nDebug get_expected_obs_factorized:")
    # print("Input state beliefs:", [q.shape for q in qs_pi[0]])
    # print("First modality A shape:", A[0].shape)
    # print("A_factor_list:", A_factor_list)
...
def calc_expected_utility(qo_pi, C):
    ...
    for t in range(n_steps):
        ...
        # for modality in range(num_modalities):
        #     lnC = spm_log_single(C_prob[modality][:, t])
        #     modality_util = qo_pi[t][modality].dot(lnC)
        #     timestep_util += modality_util
        #     if t == 0:  # Only print first timestep to avoid spam
        #         print(f"Modality {modality} utility: {modality_util:.2f}")
        # if t == 0:
        #     print(f"Timestep {t} total utility: {timestep_util:.2f}")
        # expected_util += timestep_util

    # print(f"Total utility across all timesteps: {expected_util}")
...
def calc_states_info_gain_factorized(A, qs_pi, A_factor_list):
    ...
    # # print(f"A_Factor_List: {A_factor_list}")
    # print("\nSearch modality A matrices:")
    # for m in range(3, 6):  # Modalities 3,4,5
    #     print(f"\nModality {m} A matrix:")
    #     print(f"Shape: {A[m].shape}")
    #     print("Full matrix:")
    #     print(A[m])
    #     print("\nColumn-wise probabilities:")
    #     for col in range(A[m].shape[1]):
    #         print(f"Column {col} sum: {np.sum(A[m][:,col])}")
    #         print(f"Column {col} values: {A[m][:,col]}")
    ...
    for t in range(n_steps):
        for m, A_m in enumerate(A):
            ...
            # print(f"States Surprise: {states_surprise}")

    # for t in range(n_steps):
    #     timestep_surprise = 0
    #     for m, A_m in enumerate(A):
    #         factor_idx = A_factor_list[m]
    #         factor_surprise = spm_MDP_G(A_m, qs_pi[t][factor_idx])
    #         timestep_surprise += factor_surprise
    #         if t == 0:  # Only print first timestep
    #             print(f"Modality {m} info gain (factors {factor_idx}): {factor_surprise:.2f}")
    #     if t == 0:
    #         print(f"Timestep {t} total info gain: {timestep_surprise:.2f}")
    #     states_surprise += timestep_surprise

    # print(f"Total info gain across all timesteps: {states_surprise:.2f}")
    return states_surprise
...
def sample_action(q_pi, policies, num_controls, action_selection="deterministic", alpha = 16.0):
    ...
    # Get marginal action probabilities properly 
    for pol_idx, policy in enumerate(policies):
        # print(f"pol_idx: {pol_idx}, policy: {policy}")
        for factor_i, action_i in enumerate(policy[0]): # Note the [0] to get first timestep
            action_marginals[factor_i][int(action_i)] += q_pi[pol_idx]
            # print(f"Action Marginals: {action_marginals}")
    ...
    for factor_i in range(num_factors):
        # print({f"Factor i: {factor_i}, Action Marginals Factor i: {action_marginals[factor_i]}"})
        if action_selection == 'deterministic':
            selected_policy[factor_i] = np.argmax(action_marginals[factor_i])
        elif action_selection == 'stochastic':
            selected_policy[factor_i] = utils.sample(softmax(action_marginals[factor_i] * alpha))
...
def sophisticated_inference_search(qs, policies, A, B, C, A_factor_list, B_factor_list, I=None, horizon=1,
    ...
    # print("\nDEBUG - Policy inference:")
    # print("Number of valid policies:", len(policies))
    # print("First policy actions:", policies[0])

    for idx, policy in enumerate(policies):

        # if idx < 3:
        #     print(f"\nDEBUG - Policy {idx}:")
        #     print("Policy actions:", policy)
        #     print("qs_pi type:", type(qs_pi[idx]))
        #     if qs_pi[idx] is not None:
        #         print("qs_pi contents:", qs_pi[idx])
        ...
        # # Debug prints
        # print(f"\nFor policy {idx}: {policy}")
        # print("Expected states:")
        # for f, states in enumerate(qs_pi[idx]):
        #     print(f"Factor {f}:", states)
        # print("\nExpected observations:")
        # for m, obs in enumerate(qo_pi[idx][0]):  # Look at first timestep
        #     print(f"Modality {m}:", obs)

        G[idx] += calc_expected_utility(qo_pi[idx], C)
        # print(f"Utility contribution to G: {calc_expected_utility(qo_pi[idx], C)}")

        G[idx] += calc_states_info_gain_factorized(A, qs_pi[idx], A_factor_list)

        # info_gain = calc_states_info_gain_factorized(A, qs_pi[idx], A_factor_list)
        # print(f"Info gain contribution to G: {info_gain}")
