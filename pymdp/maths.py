...
def spm_MDP_G(A, x):
    ...
    qo = 0
    # idx = np.array(np.where(qx > np.exp(-16))).T
    idx = np.array(np.where(qx > np.exp(-64))).T # Changed this from original PyMDP Code
    ...
    if utils.is_obj_array(A):
        ...
        for i in idx:
            ...
            qo += qx[tuple(i)] * po
            # G += qx[tuple(i)] * po.dot(np.log(po + np.exp(-16)))
            G += qx[tuple(i)] * po.dot(np.log(po + np.exp(-64))) # Changed this from original PyMDP Code
    ...
    G = G - qo.dot(spm_log_single(qo + np.exp(-32)))  # Changed this from original PyMDP Code

    # print(f"Subtracting: {qo.dot(spm_log_single(qo))}")
    # print(f"G3: {G}")
